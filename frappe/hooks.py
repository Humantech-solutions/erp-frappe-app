...
def handle_exception(e,r,data):
               r.publish("exception",json.dumps(
                      {
                              'message':str(e),
                      **data
					  }
...
def redis_listner():
    ...
    for message in pubsub.listen():
        if message['channel'] == b'vehicle' and message['type'] == 'message':
                  data = json.loads(message['data'].decode("utf-8"))
                  if 'contract_id' in data and 'action' in data:
                         try:
                                d = frappe.get_doc('Contracts',data['contract_id'])
                                frappe.model.workflow.apply_workflow(d,data['action'])
                         except Exception as e:
                                handle_exception(e,r,data)
                  else:
                         vechile_id = data['vehicle_id']
                         __doc = frappe.db.get_list('Contracts',filters={'vehicle_id':vechile_id, 'status':('not in',['Cancelled','Completed'])})
                         print(bool(__doc),"#")
                         if bool(__doc) and 'action' in data:
                                 try:
                                         _doc = frappe.get_doc('Contracts',__doc[0]['name'])
                                         frappe.model.workflow.apply_workflow(_doc,data['action'])
                                 except Exception as e:
                                         handle_exception(e,r,data)
                         elif bool(__doc):
                                 r.publish('contracts',json.dumps({
                                         'message':'Active Contract Already Exists for {}, and action not passed'.format(vechile_id),
										 **data
								 }))
                         else:
                                 doc = frappe.get_doc({
