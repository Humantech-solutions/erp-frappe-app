...
def export_customizations(
    ...
    custom = {
        "custom_fields": frappe.get_all(
            "Custom Field", fields="*", filters={"dt": doctype}
        ),
        "property_setters": frappe.get_all(
            "Property Setter", fields="*", filters={"doc_type": doctype}
        ),
        "custom_perms": [],
        "links": frappe.get_all(
            "DocType Link", fields="*", filters={"parent": doctype}
        ),
        "doctype": doctype,
...
def sync_customizations_for_doctype(data: dict, folder: str, filename: str = ""):
    ...
    def sync(key, custom_doctype, doctype_fieldname):
        ...
        for doc_type in doctypes:
            ...
            if doc_type == doctype or not os.path.exists(
                os.path.join(folder, scrub(doc_type) + ".json")
            ):
    ...
    if not frappe.db.exists("DocType", doctype):
        ...
        print(
            _("Skipping fixture syncing for doctype {0} from file {1}").format(
                doctype, filename
            )
        )
        return
...
def get_doctype_module(doctype: str) -> str:
    ...
    doctype_module_map = frappe.cache.get_value(
        "doctype_modules",
        generator=lambda: dict(
            frappe.qb.from_("DocType").select("name", "module").run()
        ),
    )
...
def get_module_app(module: str) -> str:
    ...
    if app is None:
        frappe.throw(
            _("Module {} not found").format(module), exc=frappe.DoesNotExistError
        )
    return app
...
def get_collections():
	...
	return collection
	""".format(
                    doc.get("connection_uri"),
                    doc.get("module").replace(" ", "").replace("-", ""),
                    n_name,
                )
